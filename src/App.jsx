...
export default function App() {
  ...
  const handleAudit = async () => {
    ...
    try {
      ...
      if (wrapper.status !== 200) throw new Error("Anthropic error " + wrapper.status + ": " + wrapper.raw);
      const data = JSON.parse(wrapper.raw);
      const raw = data.content?.find(b=>b.type==="text")?.text || "";
      if (!raw) throw new Error("No content in response: " + wrapper.raw);
